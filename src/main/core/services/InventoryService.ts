...
export class InventoryService {
  ...
  sellProduct(input: SellInput): { product: Product; mapping: ProductPlatformMapping } {
    const product = this.products.findById(input.productID)
    if (!product) throw new Error(`商品「${input.productID}」不存在。`)
    if (!this.platforms.findById(input.platformID)) throw new Error(`平台「${input.platformID}」不存在。`)
    if (input.quantity <= 0) throw new Error('賣出數量需大於 0。')

    // Merge into the same day's record if one already exists on this platform.
    const mapping = this.mappings.createOrMergeDaily({
      ProductID: input.productID,
      PlatformID: input.platformID,
      DateRecorded: input.date || new Date().toISOString(),
      PlatformTitle: '-',
      PlatformDescription: '-',
      SellingPrice: input.sellingPrice ?? 0,
      SalesVolume: input.quantity
    })

    const newStock = Math.max(0, product.StockQuantity - input.quantity)
    const updated = this.products.update(input.productID, { StockQuantity: newStock })!
    return { product: updated, mapping }
  }
}
